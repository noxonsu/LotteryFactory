...
class LotteryFactory_Meta_Box {
    ...
    $lottery_at_homepage = get_option( 'lotteryfactory_id_at_homepage', 'false');
		// Form fields.
    ?>
    <div class="nav-tab-wrapper lottery-nav-tabs wp-clearfix">
      <a href="#lottery-tab-main" class="nav-tab nav-tab-active"><?php esc_html_e( 'Main settings', 'lotteryfactory' ); ?></a>
      <a href="#lottery-tab-customhtml" class="nav-tab"><?php esc_html_e( 'Custom HTML', 'lotteryfactory' ); ?></a>
    </div><!-- .nav-tab-wrapper -->
    
    <div class="lottery-panel-tab panel-tab-active" id="lottery-tab-main">
      <span class="lottery-tab-header">Main settings and lottery control</span>
    ...
    <?php include "metabox_customhtml.php"; ?>
    ...
    <script type="text/javascript">
      /* GUI - Tab control */
      (($) => {
        $('.lottery-nav-tabs > a').on( 'click', function(e) {
          e.preventDefault()
          var tab = $(this).attr('href')
          $('.lottery-nav-tabs > a').removeClass('nav-tab-active')
          $(this).addClass('nav-tab-active')
          $('.lottery-panel-tab').removeClass('panel-tab-active')
    ...
    $post_meta_keys = array(
      ...
      /* Custom html code */
      'custom_html_before_head_close'   => 'lottery_custom_html_before_head_close',
      'custom_html_after_body_open'     => 'lottery_custom_html_after_body_open',
      'custom_html_before_body_close'   => 'lottery_custom_html_before_body_close'
    );
    /* dont-satinize - allow html code */
    $dont_satinize = array(
      'lottery_custom_html_before_head_close',
      'lottery_custom_html_after_body_open',
    ...
    foreach( $post_meta_keys as $metaKey => $postKey) {
      $postValue =
        isset( $_POST[ $postKey ] )
        ? (in_array($postKey, $dont_satinize))
          ? esc_html( wp_unslash( $_POST[ $postKey ] ) )
