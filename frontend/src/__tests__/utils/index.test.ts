...
describe('utils', () => {
  describe('#getBscScanLink', () => {
    // Implementation reads from window.SO_LotteryConfig.etherscan (set to testnet in test setup)
    const BASE = window.SO_LotteryConfig.etherscan
    it('correct for tx', () => {
      expect(getBscScanLink('abc', 'transaction', ChainId.MAINNET)).toEqual(`${BASE}/tx/abc`)
    })
    it('correct for token', () => {
      expect(getBscScanLink('abc', 'token', ChainId.MAINNET)).toEqual(`${BASE}/token/abc`)
    })
    it('correct for address', () => {
      expect(getBscScanLink('abc', 'address', ChainId.MAINNET)).toEqual(`${BASE}/address/abc`)
    })
    it('enum', () => {
      expect(getBscScanLink('abc', 'address', ChainId.TESTNET)).toEqual(`${BASE}/address/abc`)
    })
  })
